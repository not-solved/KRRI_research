...
#       두 지점 간 거리 계산
def get_distance(lc_location, ble_location):
    return math.sqrt(
        math.pow(lc_location[0] - ble_location[0], 2) +
        math.pow(lc_location[1] - ble_location[1], 2)
    )


#       통합 데이터 읽고 분석하기
File_read = open('signal_record_03_no_outlier.csv', 'r', encoding='utf-8')
...
for content in rd:
    ...
    #   거리계산 결과 기준 반경 내 위치하면 기존 거리 저장
    dist = get_distance(lc_result, ble_result)
    if dist <= DIST_BASE:
