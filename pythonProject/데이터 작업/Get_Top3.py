...
for i in filelist:
    ...
    fwrite = open(i[:6] + '_GetMaxResult.csv', 'w', encoding='utf-8', )
    ...
    minor_avg_list.append([i[7:9], Max_MINOR, Max_RSSI])

    fread.close()

wr.writerow(['Base', 'result minor', 'Max Rssi'])
for result in minor_avg_list:
    wr.writerow(result)
    print(result)
