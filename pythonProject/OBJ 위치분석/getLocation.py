...
for file in fileList:
    ...
    for line in rd:
        ...
        #   현재까지 기록된 로드셀 정보를 취합한다
        if line[0] == "Break":

            #   35, 36, 37, 38, 39, 40, 56, 57, 60번 각 블록의 로드셀 데이터를
            #   우선순위 큐에 기록
            for m in range(0, 9):
                for i in range(0, 4):
                    record_Queue(Q, lcValue[m][i] / lcValue[m][4], get_coord(minorList[m], i+5))

            #   이후 상위 N개 블록을 선정하여 예상 좌표를 계산
            resultCoord = get_target_coord(Q)
            coordList.append(resultCoord)
            iterList.append(file[4:5] + "_" + str(breakCnt))
            print(file, breakCnt, " : ", resultCoord)
            breakCnt += 1
            initialize()

            #   이후 다른 자세로 밟은 로드셀 정보를 읽는다.
            continue
...
#       산점도를 이용하여 좌표 시각화
i = 0
for result in coordList:
    plt.figure(figsize=(7, 7))
    plt.axis([-0.5, 5.5, -0.5, 5.5])
    plt.xticks(np.array([-0.5, 0.5, 1.5, 2.5, 3.5, 4.5, 5.5]), labels=['-0.5', '0.5', '1.5', '2.5', '3.5', '4.5', '5.5'])
    plt.yticks(np.array([-0.5, 0.5, 1.5, 2.5, 3.5, 4.5, 5.5]), labels=['-0.5', '0.5', '1.5', '2.5', '3.5', '4.5', '5.5'])
    plt.scatter(result[0], result[1], s=500)
    plt.grid(True)
    plt.savefig("fig_" + iterList[i] + ".png")
    i += 1
    ...
    for line in rd:
        ...
        if line[0] != time:
            if line[0] == 'Break':
                ...
                plt.figure(figsize = (7, 7))
                plt.scatter(x_IterList, y_IterList)
                plt.axis([-0.5, 5.5, -0.5, 5.5])
                plt.show()
            ...
            resultCoord = get_target_coord(Q)
            print(file, breakCnt, " : ", resultCoord)
            x_IterList.append(resultCoord[0])
            y_IterList.append(resultCoord[1])
            initialize()
            continue
